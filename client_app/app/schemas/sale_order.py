    ...
    def to_odoo_payload(self) -> List[dict]:
        ...
        for order in self.root:
            ...
            order_dict["order_line"] = [
                (0, 0, line) for line in order_dict.get("order_line", [])
            ]
            ...
            "example": [
                {
                    "partner_id": 1,
                    "order_line": [
                        {"product_id": 23, "product_uom_qty": 2, "price_unit": 50000},
                        {"product_id": 23, "product_uom_qty": 1, "price_unit": 75000},
                    ],
                }
            ]
        }
    ...
    model_config = {
        "json_schema_extra": {
            "example":{
                "partner_id": 1,
                "order_line": [
                    {"id": 1, "product_id": 23, "product_uom_qty": 2, "price_unit": 50000},
                    {"id": 2, "product_id": 23, "product_uom_qty": 1, "price_unit": 75000},
                ],
            }
        }
    }
...
class SaleOrderSearchRequest(BaseModel):
    ...
    fields: List[str] = Field(default_factory=lambda: ["id", "name"], example=["id", "name", "partner_id", "amount_total"])
